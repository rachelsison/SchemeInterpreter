...
def apply_primitive(procedure, args, env):
    ...
    try:
        ...
        raise SchemeError("Incorrect procedure")
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        env.bindings[target[0]] = do_lambda_form(valls, env)  
        return target[0]
    else:
...
def check_formals(formals):
    ...
    if not scheme_listp(formals):
        raise SchemeError("badly formed expression: " + str(formals))
