...
def apply_primitive(procedure, args, env):
    ...
    if procedure.use_env:
        lst.append(env)
    try:
        return procedure.fn(*tuple(lst))
    
    except:
        raise SchemeError("You're an idiot")
...
class Frame(object):
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError("not enough arguments given")
        a = 0
        while a <= (len(vals) - 1):
            frame.bindings[formals[a]] = vals[a]
            a += 1
        return frame
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        check_form(vals, 2, 2)
        env.bindings[target] = scheme_eval(vals.second.first, env)
        return target
    elif isinstance(target, Pair):
        "*** YOUR CODE HERE ***"
        if isinstance(target.first, (int, float, complex)):
            raise SchemeError("SchemeError")
        if scheme_symbolp(target.first):
            #check_form(vals, 2)
            valls = Pair(target.second, vals.second)
            env.define(target.first, do_lambda_form(valls, env))
            return target.first
    else:
...
def do_and_form(vals, env):
    ...
    last = None
    for i in vals:
        if scheme_true(scheme_eval(i, env)) == False:
            return False
        last = i
    return i 
...
def check_formals(formals):
    ...
    while len(formals) > 0:
        if (not scheme_symbolp(formals.first)):
            raise SchemeError(str(formals.first) + 'not a scheme symbol')
        elif (formals.first in symbol_list):
            raise SchemeError('can not assign to values to one key')
